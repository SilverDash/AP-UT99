...
class UT99JSONToTextParser(JSONtoTextParser):
    def _handle_color(self, node: JSONMessagePart):
        return self._handle_text(node)  # No colors for the in-game text


class UT99CommandProcessor(ClientCommandProcessor):
    def _cmd_ut99(self):
        """Check UT Connection State"""
        if isinstance(self.ctx, UT99Context):
            logger.info(f"UT99 Status: {self.ctx.get_ut99_status()}")


class UT99Context(CommonContext):
    command_processor = UT99CommandProcessor
    ...
    def __init__(self, server_address, password):
        ...
        self.gamejsontotext = UT99JSONToTextParser(self)
    ...
    async def server_auth(self, password_requested: bool = False):
        if password_requested and not self.password:
            await super(UT99Context, self).server_auth(password_requested)

        await self.get_username()
        await self.send_connect()

    def get_ut99_status(self) -> str:
        if not self.is_proxy_connected():
            return "Not connected to Unreal Tournament"
        return "Connected to Unreal Tournament"
            ...
            async for data in websocket:
                ...
                for msg in decode(data):
                    if msg["cmd"] == "Connect":
                        ...
                        if msg["game"] != "Unreal Tournament":
                            logger.info("Aborting proxy connection: game is not UnrealTournament")
...
async def on_client_connected(ctx: UT99Context):
...
async def proxy_loop(ctx: UT99Context):
    try:
        ...
        logger.exception(e)
        logger.info("Aborting the Unreal Tournament Proxy Client due to errors")
...
def launch():
    async def main():
        ...
        logger.info("Starting the Unreal Tournament proxy server")
        ctx.proxy = websockets.serve(functools.partial(proxy, ctx=ctx),
                                     host="localhost", port=2341, ping_timeout=999999, ping_interval=999999)
